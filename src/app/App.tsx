...
import imgInvoraLogo from "@/imports/LoginInvoraLogoRefinadoPro2X/6373d67a0bacdd532ea6497bf67e00798f9d315b.png";

/* MARKER-MAKE-KIT-INVOKED */

// Paleta extraída del logo: azul profundo del orbe + cyan de las líneas de red
const C = {
  left:      "#060c18",   // panel izquierdo — azul casi negro, frío
  right:     "#0c1828",   // panel derecho — tono más cálido, contraste deliberado
  input:     "#0f1e34",   // fondo de inputs
  border:    "rgba(0,196,232,0.13)",
  borderFocus: "rgba(0,196,232,0.5)",
  cobalt:    "#1b3eb5",   // azul cobalto del orbe, más limpio
  cyan:      "#00c4e8",   // cyan de las líneas del logo
  cyanMid:   "#4dd8ee",
  cyanFaint: "rgba(0,196,232,0.45)",
  silver:    "#96b0c0",
  silverFaint: "rgba(150,176,192,0.45)",
  white:     "#edf5fa",
  error:     "#f87171",
};
...
// ─── Formulario ──────────────────────────────────────────────────────────────
function FormularioAcceso({ alIngresar }: { alIngresar: () => void }) {
  ...
  const enviar = (e: React.FormEvent) => {
    ...
    setError("");
    setCargando(true);
    setTimeout(() => { setCargando(false); alIngresar(); }, 850);
  };
...
export default function App() {
  ...
  if (ingresado) {
    return (
      <div className="size-full flex items-center justify-center" style={{ background: C.left }}>
        <div className="text-center">
          <Constelacion size={100} opacity={0.35} />
          <p className="mt-4" style={{ fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 15, fontWeight: 600, color: C.silver }}>
  ...
  return (
    <div className="flex size-full" style={{ background: C.left }}>
      <PanelMarca />
      <FormularioAcceso alIngresar={() => setIngresado(true)} />
    </div>
  );
}
